...
// Lazy initialization of GoogleGenAI to ensure it picks up the environment variable
...
apiRouter.get("/planner", async (req, res) => {
    try {
        ...
        let plannerRows: any;
        if (weekId) {
            [plannerRows] = await pool.query("SELECT * FROM planner WHERE weekId = ? ORDER BY dayIndex ASC", [weekId]);
        } else {
            [plannerRows] = await pool.query("SELECT * FROM planner ORDER BY dayIndex ASC");
        }
...
apiRouter.post("/tasks/smart-sync", async (req, res) => {
    try {
        ...
        for (const task of tasks) {
              await pool.query(`
                INSERT OR IGNORE INTO daily_tasks (id, dateStr, text, isCompleted, isSmartTask)
                VALUES (?, ?, ?, 0, 1)
