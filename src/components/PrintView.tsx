...
export const PrintView: React.FC<PrintViewProps> = ({ planner = [], recipes = [], weekId, weekDates = [], onClose }) => {
  const handlePrint = () => {
    window.print();
  };

  if (!planner || !recipes) return null;

  return (
    <div className="fixed inset-0 z-[9999] bg-white overflow-auto print-wrapper print:static print:block print:h-auto print:overflow-visible print:p-0" id="print-view-modal">
        ...
        <div className="print-container bg-white flex flex-col gap-4 min-h-[90vh]">
          <div className="text-center border-b border-gray-800 pb-2 flex justify-between items-end">
            <div>
              ...
              <p className="text-gray-500 font-mono text-[10px] uppercase tracking-widest text-left mt-1">
                Wochenmenü & Helfer (Woche {weekId.split('W')[1]})
              </p>
            </div>
            <div className="text-[10px] text-gray-400 font-mono">
              {weekDates && weekDates.length >= 7 ? `${weekDates[0].toLocaleDateString('de-DE')} - ${weekDates[6].toLocaleDateString('de-DE')}` : ''}
            </div>
          </div>
            ...
            <div className="flex flex-col flex-1">
              {DAYS.map((day, dayIndex) => (
                <div key={day} className="grid grid-cols-5 border-b border-gray-300 last:border-b-0 flex-1 h-full">
                  {/* Day Column */}
                  <div className="p-2 border-r border-gray-800 bg-gray-50 flex flex-col justify-center">
                    <h2 className="text-sm font-serif font-bold text-gray-900">{day}</h2>
                    {weekDates[dayIndex] && (
                      <span className="text-[10px] text-gray-500 font-mono">
                        {weekDates[dayIndex].toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit' })}
                      </span>
                    )}
                  </div>
